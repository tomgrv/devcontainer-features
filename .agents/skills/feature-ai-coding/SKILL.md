...
- When a run opens or is asked to watch a PR, always call `subscribe_pr_activity` to receive follow-up events (comments, CI, reviews); never use `send_later`/`ScheduleWakeup` self check-ins as a substitute — see the no-scheduled-wakeups rule above.
