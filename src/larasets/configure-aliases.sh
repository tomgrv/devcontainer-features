...
art() {
    test -n "\$LARAVEL_SAIL" && test "\$LARAVEL_SAIL" -eq 1 && sail artisan "\$@" || php -d xdebug.mode=off \${containerWorkspaceFolder:-.}/artisan "\$@"
}
...
init() {
    test -f \${containerWorkspaceFolder:-.}/artisan || return 1
