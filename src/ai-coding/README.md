...
- `.claude/settings.json` — seeds `ai-coding.json`'s `plugins` (`caveman`, `ponytail`, `superpowers`) via `enabledPlugins`/`extraKnownMarketplaces`; kept in sync by `configure-skills.sh sync` as plugins are added/removed.
