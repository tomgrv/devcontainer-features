...
Other features can opt into skills too: `dependsOn` this feature, ship a `stubs/ai-coding.json` fragment with their own `skills`/`agents`/`plugins`, and call `.claude/hooks/configure-skills.sh sync` from their own `postCreateCommand`. `zz_feature -c`'s stub deployment merges every feature's `ai-coding.json` fragment (array fields are unioned) into one root file.
...
- `ai-coding.json` (repo root) — the dedicated manifest listing everything installed: `skills` (names under `.agents/skills/`), `agents` (`npx skills` target ids — `claude-code`, `github-copilot`), and `plugins` (Claude Code plugin-marketplace entries — name, marketplace, git URL). Add or drop a skill/agent/plugin here, not in the scripts. Other features that `dependsOn` `ai-coding` can ship their own `stubs/ai-coding.json` fragment — `zz_feature -c`'s generic JSON-stub merge unions it into this same root file.
