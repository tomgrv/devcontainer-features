...
- `plugins` — Claude Code plugin-marketplace entries (`caveman`, `ponytail`), kept live in `.claude/settings.json`'s `enabledPlugins`/`extraKnownMarketplaces` by `_skills.sh sync`.

Any other installed feature that `dependsOn` `ai-coding` and ships its own `ai-coding.json` stub fragment gets it merged into this same root file (array fields are unioned) — so its skills/agents/plugins install the same way, no extra script needed beyond calling `.claude/hooks/_skills.sh sync` from its own `postCreateCommand`.
