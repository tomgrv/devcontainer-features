...
get_schema() {
    ...
    if test -f "$schema"; then
        echo "${lvl}${Green}Loading schema ${UGreen}$schema${End}" >&2
        schema=$(load_json_schema $schema)
    fi

    # if schema is a url, download it
    if test -n "$(echo $schema | grep -E '^http')"; then
        echo "${lvl}${Green}Downloading schema ${UGreen}$schema${End}" >&2
    ...
    if test -n "$allow"; then
        echo "${lvl}${Purple}Additional properties allowed at root level${End}" >&2
...
validate() {
    ...
    if test -f "$json"; then
        echo "${lvl}${Green}Loading json ${UGreen}$json${End}" >&2
        json=$(load_json $json)
    fi
    ...
    # Log
    echo "${lvl}${Blue}Parsing schema <${path:-.}> == <${real:-.}>${None} at level <$level>${End}" >&2
    ...
    for entry in '"$id"' '"not"' '"oneOf"' '"allOf"' '"anyOf"' '"type"' '"required"' '"$ref"' '"properties"' '"items"' '"additionalProperties"'; do
        ...
        echo "${lvl}- Processing <${Blue}${path}:$entry${None}>${End}" >&2
        ...
        \"\$id\")
            ...
            #log
            echo "${lvl}- Processing ${BWhite}\$id${End}" >&2

            # get id from schema
            id=$(get_json "$path.$entry" <<<"$schema" || echo ".")

            # log
            echo "${lvl}- Resolving ${Yellow}$id${End}" >&2
            ;;
        ...
        \"\$ref\")
            ...
            echo "${lvl}- Ref is ${Yellow}${ref}${End}" >&2
            echo "${lvl}  From ${Yellow}${id}${None}...${End}" >&2
            ...
            if test -n "$(echo $uri | grep -E '^http')"; then
                echo "${lvl}- Loading ${Yellow}${uri}${None}...${End}" >&2
                ...
                if test -f "$uri"; then
                    ...
                    echo "${lvl}- Loading ${Yellow}$(dirname $id)/$uri${None}...${End}" >&2
            ...
            if ! validate "$json" "$schema" "$(echo $fgt | tr '/' '.')" "$real" "$path" "$not" "$level"; then
                echo "${lvl}- ${Yellow}Reference $ref is invalid${End}" >&2
                return 1
            fi
        ...
        \"not\")
            echo "${lvl}- ${Purple}Start Not${End}" >&2

            # Parse sub schema with not flag
            if ! validate "$json" "$schema" "$path.$entry" "$real" "$path.$entry" "!" "$level"; then
                echo "${lvl}- ${Yellow}Condition is invalid${End}" >&2
                return 1
            fi

            echo "${lvl}- ${Purple}End Not${End}" >&2
        ...
        \"oneOf\" | \"allOf\" | \"anyOf\")
            ...
            echo "${lvl}- ${Cyan}Start $entry loop${End}" >&2
            ...
            echo "${lvl}- ${Cyan}End $entry loop ($count/$size)${End}" >&2
        ...
        \"type\")
            ...
            # Log
            echo "${lvl}- Processing ${BWhite}Type${End}" >&2
            ...
            if test -z "$type"; then
                echo "${lvl}- ${Yellow}Property ${real:-.} is ${type_json}, not of expected type ${type_schema}${End}" >&2
                return 1
            fi
            ...
            # Log
            echo "${lvl}- ${Green}Property ${real:-.} is of expected type ${type}${End}" >&2
            ;;
        ...
        \"required\")
            ...
            if test "$type" != "\"object\""; then
                echo "${lvl}- ${Yellow}Skip required not an object ($type)${End}" >&2
                continue
            fi
            ...
            for prop in $(get_array_items "$path.$entry" <<<"$schema"); do
                ...
                if is_existing_path "$real.$prop" <<<"$json"; then
                    if test -n "$not"; then
                        echo "${lvl}- ${Yellow}Property $prop is present but not required${End}" >&2
                        return 1
                    fi
                else
                    if test -z "$not"; then
                        echo "${lvl}- ${Yellow}Property $prop is missing but required${End}" >&2
                        return 1
                    fi
                fi

                echo "${lvl}- ${Green}Property requirement is valid for $prop${End}" >&2
            done
            ;;
        ...
        \"items\")
            ...
            if test "$type" != "\"array\""; then
                echo "${lvl}- ${Yellow}Skip properties not an  ($type)${End}" >&2
                continue
            fi

            # Log
            echo "${lvl}- Processing ${BWhite}Items${End}" >&2
            ...
            for item in $(get_keys "$real" <<<"$json" | sort); do
                ...
                # Log
                echo "${lvl}- Processing item ${Purple}$item${End}" >&2

                if ! validate "$json" "$schema" "$path.$entry" "$real.$item" "$path.$entry" "$not" "$level"; then
                    echo "${lvl}- ${Orange}Item $item is invalid${End}" >&2
                    return 1
                fi
            done
            ;;
        ...
        \"properties\")
            ...
            if test "$type" != "\"object\""; then
                echo "${lvl}- ${Yellow}Skip properties not an object ($type)${End}" >&2
                continue
            fi

            # Log
            echo "${lvl}- Processing ${BWhite}Properties${End}" >&2
            ...
            for prop in $props; do
                ...
                if $not is_existing_path "$real.$prop" <<<"$json"; then
                    ...
                    # Log
                    echo "${lvl}- Processing <${Purple}$prop${None}>${End}" >&2
                    ...
                    # Log
                    echo "${lvl}- ${Yellow}Skip property $real.$prop not present${End}" >&2
                fi
            done
        ...
        \"additionalProperties\")
            ...
            echo "${lvl}- Processing ${BWhite}Additional properties${End}" >&2
            if is_existing_path "$path.$entry" <<<"$schema"; then

                echo "${lvl}- ${Purple}Additional properties allowed for ${real:-.}${End}" >&2

                for prop in $(get_keys "$real" <<<"$json" | sort); do

                    if ! grep -q "$prop" <<<"$props"; then
                        echo "${lvl}- Adding additional property <${Purple}$real.$prop${None}>${End}" >&2
    ...
    # Log
    echo "${lvl}${Blue}End Parsing, all valid!${End}" >&2
}
