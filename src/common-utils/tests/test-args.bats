...
@test "positional (-) binds under bash's non-XSI echo (regression)" {
    run bash -c '
        eval "$(bash "'"$REPO_ROOT"'/src/common-utils/bins/_zz_args.sh" "test" caller "add" "gateway" <<-EOF
	- cmd cmd       command
	+ target target target value
EOF
)"
...
@test "+ capture binds under bash's non-XSI echo (regression)" {
    run bash -c '
        eval "$(bash "'"$REPO_ROOT"'/src/common-utils/bins/_zz_args.sh" "test" caller "hello" "world" <<-EOF
	+ msg message    remaining args
EOF
)"
