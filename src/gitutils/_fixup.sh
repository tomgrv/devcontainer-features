...
git rebase -i --autosquash $sha~ --autostash --no-verify --exec 'if test -f .git/hooks/pre-commit; then (.git/hooks/pre-commit --name-only HEAD HEAD~1 && git commit --amend --no-edit --no-verify); fi'
